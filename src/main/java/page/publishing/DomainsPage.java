...
public class DomainsPage extends PageBase {
    ...
    public boolean isSpecifiedDomainAdded() {
        WaitersUtils.getWaiter(driver, 30).until(ExpectedConditions.visibilityOfElementLocated(DOMAINSUCCESSFULCREATIONNOTIF));
        WaitersUtils.getWaiter(driver, 30).until(ExpectedConditions.invisibilityOfElementLocated(DOMAINSUCCESSFULCREATIONNOTIF));
        try {
    ...
    public boolean isSpecifiedDomainDeleted() {
        WaitersUtils.getWaiter(driver, 30).until(ExpectedConditions.visibilityOfElementLocated(DOMAINSUCCESSFULDELETIONNOTIF));
        WaitersUtils.getWaiter(driver, 30).until(ExpectedConditions.invisibilityOfElementLocated(DOMAINSUCCESSFULDELETIONNOTIF));
        try {
            return driver.findElement(By.xpath(String.format(SPECIFIEDDOMAINNAMECELL, domainName))).isDisplayed();
        } catch (NoSuchElementException ex) {
            return false;
    ...
    public void changeDomainRaiting() {
        try {
            ...
            WaitersUtils.getWaiter(driver, 30).until(ExpectedConditions.invisibilityOfElementLocated(DOMAINSUCCESSFULSETTINGSSAVENOTIF));
